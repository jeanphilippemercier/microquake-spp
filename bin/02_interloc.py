...
def process(
    ...
    whiten_corner_freqs = np.array(module_settings.whiten_corner_freqs, dtype=np.float32)

    htt = app.get_ttable_h5()
    stalocs = htt.locations
    ttable = (htt.hf["ttp"][:] * samplerate_decimated).astype(np.uint16)
    ngrid = ttable.shape[1]
    ttable_row_ptrs = np.array([row.__array_interface__["data"][0] for row in ttable])
    ...
    t6 = time()
    logger.info("data %s,samplerate_decimated %s,channel_map %s,stalocs[ikeep] %s,ttable_row_ptrs[ikeep] %s,ngrid %s,nthreads %s,debug %s,debug_file %s", data, samplerate_decimated, channel_map, stalocs[ikeep], ttable_row_ptrs[ikeep], ngrid, nthreads, debug_level, debug_file)
    ...
    ot_epoch = tools.datetime_to_epoch_sec((t0 + iot / samplerate_decimated).datetime)
    ...
    logger.info("=======================================\n")
    logger.info("VMAX over threshold (%.3f > %.3f)" % (vmax, detection_threshold))

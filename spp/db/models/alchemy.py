...
continuous_data = db.Table('continuous_data', metadata,
                           db.Column('id', db.Integer, primary_key=True),
                           ...
                           db.Column('end_time',
                                     db.TIMESTAMP(timezone=True),
                                     index=True),
                           db.Column('sensor_id', db.VARCHAR(5), index=True),
                           db.Column('expiry_time',
                                     db.TIMESTAMP(timezone=True),
                                     index=True),
                           db.Column('redis_key', db.VARCHAR(40)))

triggers = db.Table('trigger', metadata,
                    db.Column('id', db.Integer, primary_key=True),
                    db.Column('trigger_on_time',
                              db.TIMESTAMP(timezone=True),
                              index=True),
                    db.Column('trigger_off_time',
                              db.TIMESTAMP(timezone=True),
                              index=True),
                    db.Column('trigger_amplitude', db.Float, index=True),
                    db.Column('sensor_id', db.VARCHAR(5), index=True),
                    db.Column('trigger_band_id', db.VARCHAR(25), index=True))

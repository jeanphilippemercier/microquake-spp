...
def encode_fastavro(schema, json_object):
    ...
    bytes_writer = io.BytesIO()
    fastavro.schemaless_writer(bytes_writer, schema, json_object)
...
def decode_fastavro(schema, avro_object):
    ...
    bytes_reader = io.BytesIO(avro_object)
    json_object = fastavro.schemaless_reader(bytes_reader, schema)
